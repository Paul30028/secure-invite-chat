    ...
    : section === "contacts"
      ...
      : section === "invites"
        ...
        : section === "me" && local.data
          ? <MePage deviceId={engine.deviceId} data={local.data} onSettings={() => setShowSettings(true)} onRestore={(data) => void local.update(() => data)} adminGroups={engine.groups.filter((group) => group.isAdmin)} onOpenAdmin={(group) => { engine.setActiveGroupId(group.groupId); setShowAdmin(true); }} />
          : active
            ? <ChatWindow
                group={active}
