...
export function SettingsModal({ profile, status, adminAvailable, adminGroupName, onClose, onSaved, onProfileChange, onAdminAction }: {
  ...
  return <div className="settings-backdrop"><section className="settings-panel">
    ...
    <div className="settings-scroll">
      ...
      {view === "about" && <><div className="settings-card about"><img src="/brand/wheat-app-icon.png" alt="" /><h2>邀群密聊</h2><button className="version-button" onClick={tapVersion}>版本 0.2.0</button></div><div className="settings-group">{["隐私政策","开源许可","安全说明","检查更新"].map((x,i)=><button className="settings-item" key={x} onClick={()=>setMessage(i===3?"当前已是最新版本。":"将在正式发布版中提供。")}><i>{["♢","⌘","▣","↻"][i]}</i><span><b>{x}</b></span><strong>›</strong></button>)}</div>{adminMode&&<button className="primary-button" onClick={()=>setView("admin")}>进入管理员设置</button>}</>}
